 ...
 *    files the node under `incomplete`. This page paints three `radial-gradient`
 *    washes onto `<body>`, gradient fills onto `.step-badge`, `.progress-dot`
 *    and `.sp-btn-on`, and a `color-mix(in oklab, ...)` tint onto `.cl-hero-why`,
 *    so a great deal of text composites onto something other than the flat
 *    colour axe requires.
 ...
 *    `opacity: .85`, `.sp-ghost` at `.75`, and every `.exhibit` / `.agg-lane`
 *    begins its `rise` / `slideIn` entrance keyframe at `opacity: 0`.
 *
 * It also exists because nearly every surface on this page is TRANSLUCENT:
 * `.exhibit` is `rgba(30,41,59,.6)`, `.card` and `.participant` are
 * `rgba(15,23,42,.4)`, and `.attack-panel` is `var(--surface-2)` —
 * `rgba(255,255,255,.5)`. None of them is the flat colour a naive check assumes,
 * so what a ratio actually is depends on the whole stack underneath.
 *
 ...
 *    past a scroller's client box is not dimmed or partly drawn — it is absent
 *    from the frame, and asking what colour it sits on has no answer. Its rect
 *    is outside every ancestor's box, so the ancestor walk would find nothing
 *    behind it and fall through to white, inventing a failure. Skip it, and rely
 *    on the wider viewport where the very same element is measured for real.
 *
 ...
 *    never laid down. Both `<h1>`s here are exactly that case: `.cl-hero-title`
 *    paints a `linear-gradient` through `-webkit-text-fill-color: transparent`,
 *    so its `color` is not the ink on screen.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. The Shamir
 *    polynomial plot (`.sp-svg`) is a real SVG figure: it draws its panel and
 *    axes as `<rect>`/`<line>` and then its point and secret labels as following
 *    `<text>`, so a label's backdrop is a PRECEDING SIBLING shape, not an
 *    ancestor. An ancestor-only walk would composite those labels onto the svg's
 *    own transparent background and report a ratio nothing on screen has. So for
 *    SVG text, any earlier sibling shape whose box contains the text box is
 *    composited first — and SVG ink is read from `fill`, not `color`.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: this page mixes its accent
     * into surfaces with `color-mix()` in BOTH colour spaces — `.cl-hero-why` is
     * `color-mix(in oklab, var(--accent) 6%, transparent)` and the shared header
     * bar's ink and borders are `color-mix(in srgb, ...)` — and Chromium reports
     * those to `getComputedStyle` unchanged rather than converting them to sRGB. A
     ...
     * that spans its element, but this page's `<body>` paints THREE low-alpha
     * radial washes that fade to `transparent` — indigo at `50% -20%`, cyan at
     * `80% 60%`, violet at `10% 80%`. This document runs many viewports tall, so
     * judging the recap and footer at the bottom by the indigo glow above the top
     * edge would invent backdrops that text never sits on. So each gradient is *sampled* at
     ...
     *
     * This page renders up to seven participant cards, seven commitment cards and
     * seven share cards, plus a Shamir plot whose every point carries its own
     * label — hundreds of text nodes all walking the same handful of translucent
     * `.exhibit` / `.card` ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures. This page has five of
     * them at every paint: each progress step carries an `.sr-only` span reading
     * ": completed" / ": current step" / ": not started", the spoken twin of the
     * `aria-hidden` `.progress-dot` glyph beside it, and `.sr-only` is the
     * classic `clip: rect(0, 0, 0, 0)` recipe. Deliberately narrow — only a ZERO-area clip
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has two kinds: the per-participant "Show
      // secret share scalar" disclosures, which ship CLOSED, and the two
      // `.viz-details` figures, which ship `open`. The gate toggles both and
      // scans each state explicitly.
     ...
     * it, this page's decorative glyphs — the ⚔ and ★ `.step-badge`s, the 🚫 and
     * ⚠️ `.insight-icon`s, the ✓/✗ `.subset-check` — are each measured as if the
     * glyph took the element's declared `color`. An emoji paints in its own
